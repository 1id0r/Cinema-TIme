...
function highlightAvailableSeatsAround() {
  console.log('gElSelectedSeati', gElSelectedSeat.dataset.i)
  console.log('gElSelectedSeatj', gElSelectedSeat.dataset.j)
  var board = gCinema
  ...
  for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
    ...
    for (var j = colIdx - 1; j <= colIdx + 1; j++) {
      ...
      if (currCell.isSeat && !currCell.isBooked) {
        ...
        elSeat.classList.add('white')
        gElSeats.push(elSeat)
      }
    }
  }
  setTimeout(() => {
    for (var i = 0; i < gElSeats.length; i++) {
      gElSeats[i].classList.remove('white')
    }
    gElSeats = []
  }, 1000)
}
